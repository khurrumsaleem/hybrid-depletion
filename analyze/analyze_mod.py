...
mix = ['Kr83', 'Tc99', 'Ru101', 'Rh103', 'Rh105', 'Xe131', 'Xe133', 'Xe135',
...
hybrid2_results = openmc.deplete.Results(root_path.format(run_info=f"hybrid2-mod/{args.nuclides}/{egroup}"))
...
for nuc in sorted(all_nuclides):
    ...
    ax.set_title(f"{nuc}{ast}, E={egroup}, {args.nuclides} - modified")
    ax.set_xlabel("Time (days)")
    ax.set_ylabel("Concentration [atoms/b]")
    ax.legend()
    ax.grid(True, which='both')
    plt.tight_layout()
    plt.savefig(f"figures/{args.nuclides}/{egroup}/{nuc}-absolute-hy2mod.png")
    plt.close()
    ...
    ax.grid(True, which='both')
    plt.tight_layout()
    plt.savefig(f"figures/{args.nuclides}/{egroup}/{nuc}-diff-hy2mod.png")
    plt.close()
    ...
    ax.grid(True, which='both')
    plt.tight_layout()
    plt.savefig(f"figures/{args.nuclides}/{egroup}/{nuc}-relative-hy2mod.png")
    plt.close()
